...
document.addEventListener('loaded', (e) => {
  ...
  TL.timelines.main
    ...
    .from('#stars', { duration: 1.5, scale: 0, ease: 'power4.out', svgOrigin: '150px 125px' }, TL.setCurTime('screen2', 0.6))
    ...
    .set('#mentions-link', { visibility: 'visible' }, TL.setCurTime('screen3', 0))
    .to('#stars path', { duration: 0.4, scale: 3, opacity: 0, svgOrigin: '150px 125px', ease: 'none' }, TL.setCurTime('screen3', 0))
    .to('#button', { duration: 0.4, x: -93, y: -105, scale: 0.529, transformOrigin: '50% 50%', ease: 'power3.out' }, TL.setCurTime('screen3', 0))
    .from('#date', { duration: 0.4, opacity: 0, x: 50, ease: 'power3.out' }, TL.setCurTime('screen3', 0.2))
    .from('.claims', { duration: 0.8, opacity: 0, x: -30, ease: 'power3.out', stagger: 0.2 }, TL.setCurTime('screen3', 0.4))
    .from('#iphone', { duration: 2, opacity: 0, x: 30, ease: 'power4.out' }, TL.setCurTime('screen3', 0.6))
    .from('#apple', { duration: 2, opacity: 0, x: -30, ease: 'power4.out' }, TL.setCurTime('screen3', 0.6))
    .from('#CTA', { duration: 0.6, opacity: 0, y: -10, ease: 'power2.out' }, TL.setCurTime('screen3', 1.8))
    /**
     * ecran 4
     */
    .from('#ecran4', { duration: 0.3, opacity: 0 }, TL.setCurTime('screen4', 6.5))
    .to('#claim', { duration: 0.3, y: -24, ease: 'power4.out' }, TL.setCurTime('screen4', 0))
    .to('#claim1', { duration: 0.3, x: 111, y: -42, ease: 'power4.out' }, TL.setCurTime('screen4', 0))
    .to('#claim2,#claim3', { duration: 0.3, y: -41, ease: 'power4.out' }, TL.setCurTime('screen4', 0))
    .to('#iphone', { duration: 1, scale: 0.82, y: 1, ease: 'power4.out' }, TL.setCurTime('screen4', 0))
    .to('#apple', { duration: 1, scale: 0.8, x: 5, y: -3, ease: 'power4.out' }, TL.setCurTime('screen4', 0))
    .from('.argus', { duration: 0.6, opacity: 0, x: 30, ease: 'power3.inOut', stagger: 0.15 }, TL.setCurTime('screen4', 0.3))
